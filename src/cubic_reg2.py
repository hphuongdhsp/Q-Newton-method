...
class CubicRegularization(Algorithm):
    ...
    def cubic_reg(self):
        ...
        intermediate_function_values=[f(x_new)]
        while iter < self.maxiter:
            ...
            intermediate_function_values.append(f(x_new))
            iter += 1
        return x_new, intermediate_points, intermediate_function_values, iter, flag
...
class AdaptiveCubicReg(Algorithm):
    def __init__(self, x0, f, gradient=None, hessian=None, L=None, L0=None, sigma0=1, eta1=0.1, eta2=0.9, kappa_easy=0.0001, maxiter=50, submaxiter=10000, conv_tol=1e-5, hessian_update_method='exact', conv_criterion='gradient', epsilon=2*np.sqrt(np.finfo(float).eps)):
        ...
        self.intermediate_function_values=[f(self.x0)]
        self.iter = 0
    ...
    def _update_x_params(self, s, x_old, f_x):
        ...
        if rho >= self.eta1:
            ...
            self.intermediate_function_values.append(self.f(x_new))
    ...
    def adaptive_cubic_reg(self):
        ...
        return x_new, self.intermediate_points, self.intermediate_function_values, self.iter, flag
